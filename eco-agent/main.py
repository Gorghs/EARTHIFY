...
from .prompts import (
    ...
    web_searching_template,
    enviromental_suggestions,
)

from report_analysis_and_storage import firebase_helper


router = APIRouter(prefix="/eco-agent", tags=["Eco-Friendly Suggestions"])
...
async def describeProducts(userMedicalAilments: str, file: UploadFile = File(...)):
    ...
    with open(
        "/Users/macromrit/Documents/Rcube-Sustainify/media/videos/sunnyD.mp4", "wb"
    ) as jammer:
        jammer.write(file_content)

    product_details = eval(
        typeDocInputNOutputFormat(
            model,
            product_description_template,
            EdibleDataExtraction,
            "/Users/macromrit/Documents/Rcube-Sustainify/media/videos/sunnyD.mp4",
        )
    )
    ...
    pros_and_cons_enviromental = eval(
        getOutPutInFormat(
            model,
        ...
        getOutPutInFormat(
            ...
            enviromental_suggestions.render(
                ...
                user_medical_report_details="\n\n".join(
                    firebase_helper.retrieve_data_by_keyword("Om123")
                ),
            ),
            [],
            HealthProsAndCons,
